...
import com.github.artillect.voltaics.power.implementation.BaseEnergyPath;
...
public class TileEntityLowVoltageConduit extends TileEntity implements ITickable {
	private BaseEnergyPath container;
	
	public TileEntityLowVoltageConduit() {
		this.container = new BaseEnergyPath(70, 1200);
	}
	...
	public EnumConduitConnection getConnection(IBlockAccess world, BlockPos pos, EnumFacing side){
		if (world.getTileEntity(pos) instanceof TileEntityLowVoltageConduit){
			...
			if (world.getTileEntity(pos).hasCapability(Capabilities.CAPABILITY_SOURCE, side) || world.getTileEntity(pos).hasCapability(Capabilities.CAPABILITY_SINK, side)){
				return EnumConduitConnection.BLOCK;
			}
		}
		return EnumConduitConnection.NONE;
	...
	public void readFromNBT(NBTTagCompound compound){
        ...
        this.container = new BaseEnergyPath(compound.getCompoundTag("JouleContainer"));
	}
    ...
    public <T> T getCapability (Capability<T> capability, EnumFacing facing) {
        ...
        if (capability == Capabilities.CAPABILITY_PATH)
	...
	public void neighborChanged() {
		for (EnumFacing side : EnumFacing.values()) {
			...
			if (tile != null && tile.hasCapability(Capabilities.CAPABILITY_SOURCE, side)) {
				this.container.addToVoltageSourceList(tile, side);
			}
		}
	}
